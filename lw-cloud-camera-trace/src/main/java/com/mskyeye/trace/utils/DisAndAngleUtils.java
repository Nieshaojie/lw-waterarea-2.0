 ...
 * @Version:1.0
 **/
public class DisAndAngleUtils {

    private static final double M_PI = 3.14159265358979323846;

    private static final double EARTH_RADIUS = 6371.0;

    public static double HaverSin(double thera) {
    ...
    public static double get_distance(double lon1, double lat1, double lon2, double lat2) {
    ...
    /**
     * 经纬度坐标转换为方位角（第一个点是起点,第二个点是终点）
     */
    public static double get_bearing(double lat1, double lng1, double lat2, double lng2) {
        double y =  Math.sin( Math.toRadians(lng2 - lng1)) *  Math.cos( Math.toRadians(lat2));
        double x =  Math.cos( Math.toRadians(lat1)) *  Math.sin( Math.toRadians(lat2))
                -  Math.sin( Math.toRadians(lat1)) *  Math.cos( Math.toRadians(lat2)) *  Math.cos( Math.toRadians(lng2 - lng1));
        return  Math.toDegrees( Math.atan2(y, x));
    }



    public static double gis_Dis(double lat1, double lon1, double lat2, double lon2) {
    ...
    public static double gis_Angle(double lat1, double lon1, double lat2, double lon2) {
        GeodeticCalculator calc = new GeodeticCalculator();
        calc.setStartingGeographicPoint(lon1, lat1);
        calc.setDestinationGeographicPoint(lon2, lat2);
        calc.getAzimuth();
