...
/**
    ...
    public void sendTrackToPlatform(LwTrackPacket packet,Long t1) {
        try {
            ...
            // 构造批次编号（厂商id-设备号-时间戳）
            json.put("batchNumAndSerial", sourceVendor + "-" + c.getSTATIONID() + "-" + t1);
...
/**
    ...
    public void sendTrackToPlatformRD(LwTrackPacket packet,Long t1) {
        try {
            if (packet == null || packet.getITEM() == null || packet.getITEM().isEmpty()) {
                log.warn("航迹包为空，跳过发送");
                return;
